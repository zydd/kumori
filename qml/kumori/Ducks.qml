...
Item {
    ...
    Settings {
        property alias c: param.c
        property alias center: param.center
    ...
    ShaderEffect {
        ...
        property point center: param.center
    ...
    Timer {
        ...
        onRunningChanged:
            if (running) {
                ...
                print(
                        'c:', param.c, 'zoom:', param.zoom, 'center:', param.center,
        ...
        onTriggered: {
            ...
            param.c.x += param.v.x * param.map_zoom
            param.c.y += param.v.y * param.map_zoom
...
//            center_v.y *= 0.8
    ...
    function translate(dx, dy) {
        param.center.x += Math.cos(param.rotation) * dx - Math.sin(param.rotation) * dy
        param.center.y += Math.sin(param.rotation) * dx + Math.cos(param.rotation) * dy
    }
    ...
    MouseArea {
        ...
        function zoomAt(factor, pos) {
            ...
            param.center.x = param.center.x * factor + coord.x * (1-factor)
            param.center.y = param.center.y * factor + coord.y * (1-factor)
    ...
    Keys.onReleased: {
        ...
        switch(event.key) {
            ...
            case Qt.Key_G:
                ...
                param.map_size = 0
                var filename = `screenshots/irz,${param.c.x},${param.c.y},${param.center.x},${param.center.y},${param.zoom}.png`
        ...
        switch(event.key) {
            ...
            case Qt.Key_Period:
                ...
                var t
                t = param.center; param.center = param.c; param.c = t
    ...
    function preset() {
        param.c = Qt.point(-0.623048, 4.61666)
        param.center = Qt.point(-2.61087, 0.40818)
        param.zoom = 0.12224574942781402
        param.iter = 181
        param.pre = 0
        param.col = 0.98236
        param.col_shift = 0.15999999999999998
        param.rotation = 0
        param.map_zoom = 0.12
        param.multisample = 4
        param.main_image = 'julia'
